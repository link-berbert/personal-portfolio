// Lincoln Berbert — Portfolio data

export const CREATIVE_WORK = [
  {
    id: "berby",
    num: "01",
    title: "Berby Music.",
    discipline: "Music · Identity",
    year: "2022 —",
    role: "Artist, producer",
    summary:
      "Berby is an ongoing music and creative direction project with its own sound, aesthetic, and internal logic. Over time, it has grown into a cohesive creative brand.",
    tags: ["Music", "Identity", "Worldbuilding"],
    externalLink: {
      href: "https://beacons.page/berbymusic",
      label: "View Berby Music Website ↗",
    },
  },
  {
    id: "james-judas",
    num: "02",
    title: "James Judas.",
    discipline: "Character · Narrative",
    year: "2024 —",
    role: "Author, designer",
    summary:
      "James Judas is a long-form narrative identity built around ambition, belief, betrayal, and the cost of self-creation. The project follows a man divided against himself, someone driven by the pursuit of greatness until that same hunger turns inward and begins to consume him. It lives in the tension between thesis, antithesis, and synthesis: vision and ego, devotion and corruption, the desire to become something greater and the risk of being destroyed by it.",
    bodySubtitle: "Part of the Berby brand.",
    tags: ["Narrative", "Music", "Identity"],
  },
  {
    id: "ghost-project-ix",
    num: "03",
    title: "Ghost Project IX.",
    discipline: "Worldbuilding · Ambient · AI · Film",
    year: "2025 —",
    role: "Worldbuilder, composer",
    summary:
      "Ghost Project IX is a far-future science fiction world set after ecological collapse, solar catastrophe, and humanity’s exodus from a dying solar system. It moves through industrial decay, orbital ruins, Martian necropolises, solitary deep-space travel, and distant sanctuaries around Proxima Centauri. The project is an experiment in creative direction, visual design, and worldbuilding, using AI-generated music to explore memory, survival, technology, and the role of the human creator in a post-human world.",
    tags: ["Worldbuilding", "Ambient", "AI", "Film"],
    externalLink: {
      href: "https://www.youtube.com/@GhostProjectIX",
      label: "View Ghost Project IX YouTube Channel ↗",
    },
  },
];

export const AI_WORK = [
  {
    id: "lightwrk",
    num: "01",
    title: "LightWrk.",
    discipline: "Company · Physical AI",
    year: "2026 —",
    role: "Founder, CEO",
    summary:
      "LightWrk is a venture-scale company building human-in-the-loop evaluation systems for physical AI and world model labs. Distilling human experience and knowledge into machines creates a theoretically infinite return on investment. This opens the door to expansive, long-horizon projects that are only limited by human imagination. The challenge lies in keeping machines consistently aligned with human values.",
    tags: ["Physical AI", "Evaluation", "Infrastructure"],
    capabilities: [
      ["Ontology design", "Structured failure taxonomies for embodied AI systems. Definin the vocabulary that makes evaluation possible."],
      ["Evaluation frameworks", "Scored, repeatable judgment frameworks and rubrics. Expert human feedback made measurable and consistent enough to train on."],
    ],
    externalLink: {
      href: "https://lightwrk.ai",
      label: "View LightWrk Website↗",
    },
  },
  {
    id: "final-hour",
    num: "02",
    title: "Final Hour.",
    discipline: "Studio · Worldbuilding",
    year: "2022 —",
    role: "Founder, creative director",
    summary:
      "Final Hour is a multidisciplinary studio built around long-horizon worldbuilding, aesthetic systems, and ambitious creative direction. It develops projects across music, film, interactive media, design, live experience, and future-facing cultural work. Every project is meant to evoke emotion and be built around a coherent world.",
    tags: ["Studio", "Worldbuilding", "Creative systems"],
    externalLink: {
      href: "https://www.linkedin.com/company/final-hour-productions/",
      label: "View Final Hour Productions LinkedIn Page ↗",
    },
  },
  {
    id: "creators-catalyst",
    num: "03",
    title: "The Creator's Catalyst.",
    discipline: "Strategy · Worldbuilding",
    year: "2025 —",
    role: "Founder, strategist",
    summary:
      "The Creator’s Catalyst is a creative strategy and worldbuilding agency for artists and founders developing deeper identities, stronger concepts, and more coherent bodies of work. It helps turn scattered ideas into integrated creative systems by refining positioning, narrative, aesthetic logic, and long-term vision.",
    tags: ["Strategy", "Worldbuilding", "Creative advisory"],
    externalLink: {
      href: "https://beacons.ai/thecreatorscatalyst",
      label: "View The Creator's Catalyst Website ↗",
    },
  },
  {
    id: "unnamed-holdings",
    num: "04",
    title: "Unnamed Holdings Co.",
    discipline: "Holding · Venture",
    year: "2026 —",
    role: "Founder, owner",
    summary:
      "Unnamed Holdings Co. is the parent structure for long-horizon ventures across technology, creative work, and future-facing systems. Built for continuity, it holds and extends a broader vision across companies, worlds, and ambitious work that may take years to fully emerge.",
    tags: ["Holding", "Structure", "Venture", "Architecture"],
  },
];
